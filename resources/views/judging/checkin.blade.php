...
>
    <section class="landing-page-section mt-6 mb-4">
        ...
        @elseif (request('bad') !== null)
            <div class="alert alert-error">The following entries were <strong>not found</strong> in the database: {{ request('bad') }}</div>
        @elseif (request('dup') !== null)
            <div class="alert alert-error"><strong>The following judging number(s) have already been assigned to entries.</strong> Please use another judging number for each: {{ request('dup') }}</div>
        @endif
        ...
        <form method="post" action="{{ route('admin.judging.checkin.store') }}">
            @csrf
            <input type="text" name="scan" id="scan" class="input input-bordered input-lg mb-2"
