...
<x-public-layout :ctx="$ctx" :show-hero="false">
    <section class="landing-page-section mt-4 mb-3">
                ...
                <tbody>
                    @foreach ($styleTypes as $type)
                        ...
                        <tr>
                            ...
                            <td>{{ $type->styleTypeBOS === 'Y' ? '<span class="text-success">Yes</span>' : '<span class="text-danger">No</span>' }}</td>
                            <td>{{ $type->styleTypeBOS === 'Y' ? ['1st place only', '1st and 2nd places', '1st, 2nd, and 3rd places'][(int) $type->styleTypeBOSMethod - 1] ?? '' : 'N/A' }}</td>
                            <td>
                                <a class="btn btn-sm btn-outline-secondary" href="{{ url('/admin/style-types/'.$type->id.'/edit') }}">Edit</a>
        ...
        @if ($row !== null || request()->routeIs('admin.style_types.create'))
            ...
            <form method="post" action="{{ url($row !== null ? '/admin/style-types/'.$row->id : '/admin/style-types') }}" class="form-horizontal">
                @csrf
                @method($row !== null ? 'put' : 'post')
                <input type="hidden" name="styleTypeOwn" value="{{ $row->styleTypeOwn ?? 'custom' }}">
                <div class="mb-3 row">
                    <label for="styleTypeName" class="col-sm-3 col-form-label">Name</label>
                    <div class="col-sm-9">
                        <input class="form-control" id="styleTypeName" name="styleTypeName" type="text" value="{{ $row->styleTypeName ?? '' }}"
                            @disabled($row !== null && $row->styleTypeOwn === 'bcoe') required>
                    </div>
                </div>
                <div class="mb-3 row">
                    <label for="styleTypeEntryLimit" class="col-sm-3 col-form-label">Entry Limit</label>
                    <div class="col-sm-9">
                        <input class="form-control" id="styleTypeEntryLimit" name="styleTypeEntryLimit" type="number" min="0" style="width:auto;" value="{{ $row->styleTypeEntryLimit ?? '' }}">
                        <span class="help-block">Limit for this style type only.</span>
                    </div>
                </div>
                <div class="mb-3 row">
                    <label class="col-sm-3 col-form-label">BOS for Style Type</label>
                    <div class="col-sm-9">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="styleTypeBOS" value="Y" id="bosYes" @checked($row === null || $row->styleTypeBOS === 'Y')>
                            <label class="form-check-label" for="bosYes">Yes</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="styleTypeBOS" value="N" id="bosNo" @checked($row !== null && $row->styleTypeBOS === 'N')>
                ...
                <fieldset class="mb-3">
                    <legend class="small">BOS Pull Method</legend>
                    @foreach ([1 => '1st place only', 2 => '1st and 2nd places', 3 => '1st, 2nd, and 3rd places'] as $value => $label)
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="styleTypeBOSMethod" value="{{ $value }}" id="bosMethod{{ $value }}"
