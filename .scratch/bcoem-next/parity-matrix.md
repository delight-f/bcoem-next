...
| index.php?section=admin&go=entries | /backoffice/entries | A | Missing legacy action controls: "Print" dropdowns (By Entry Number/Judging), "Mark All as Paid"/"Unpaid" dropdown, "All Entry Status" button, "All Participants with (Paid) Entries Email Addresses" modal, participant jump select; port keeps only filter form + table | admin/entries.admin.php |
| index.php?section=admin&go=participants | /backoffice/participants | A | Missing "Register A Participant / A Judge / A Steward" and "Assign Judges / BOS Judges / Stewards" dropdowns, "Print By Last Name", "All Participants Email Addresses" modal, "Participant Status" modal, "Updated" column | admin/participants.admin.php |
| index.php?section=admin&go=participants&filter=judges | /backoffice/participants?filter=judges | A | Same root cause as base participants (no Register/Assign/Print/Email/Status controls) plus missing "Assigned to Table(s)", "Has Entries In…", "Updated" columns | admin/participants.admin.php |
| index.php?section=admin&go=participants&filter=stewards | /backoffice/participants?filter=stewards | A | Same root cause as base participants; same missing columns | admin/participants.admin.php |
...
| index.php?section=evaluation&go=default&filter=default&view=admin | /eval | H | Legacy capture rendered an empty public shell (prefsEval off → evaluation fell through to landing page); port renders full Judging Dashboard — nothing missing in port, flag blank legacy capture | eval/dashboard.eval.php |
...
| index.php?section=admin&go=judging_scores | /admin/judging/scores | A | Score-entry UI entirely absent: "Add or Update Scores For…" dropdown, "All Tables", "View BOS Entries and Places", "Print…" BOS pullsheet, "Scores entered for 0 of 2 entries…" status; page renders only "No scores have been entered." — edit route exists but unreachable | admin/judging_scores.admin.php |
| index.php?section=admin&go=judging_scores_bos | /admin/judging/bos | A | "Add or Update…" dropdown (BOS Places Beer/Cider/Mead), "All Tables", "Print…" pullsheet/Cup Mats dropdowns missing; page renders only "No entries are eligible." — no way to enter BOS places | admin/judging_scores_bos.admin.php |
