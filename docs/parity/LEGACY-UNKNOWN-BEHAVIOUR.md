...
| U6 | Mods rendering (`prefsUseMods=Y`) on public pages | **→ PARITY-027 (2026-08-31)** — re-examined under ftp-deploy product framing: file-based extensibility is the norm, so public rendering is a feature gap, not a closed replacement. Port the render WITHOUT arbitrary PHP include (DB-stored content, legacy placement/gating). |
...
| U15 | Language toggle scope for scabs | **→ PARITY-026 (2026-08-31)** — promoted from DEFERRED: ftp-deploy/setup-anywhere means language MUST be settable. Legacy ships 6 lang packs + navbar toggle; the port stores the prefs but never applies them (English-only). Port the packs + toggle. |
...
| U20 | `custom_competition_info.pub.php` drop-in | **→ PARITY-028 (2026-08-31)** — re-examined: deploy-time drop-in is the norm under ftp model. Port a safe DB-stored equivalent + "Other Info" nav item. |
