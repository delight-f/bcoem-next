...
final class StaffPointsController extends Controller
{
    ...
    private function xmlReport(TenantContext $ctx, Request $request): Response
    {
        ...
        $output .= "\t</Comments>\n";
        $output .= "\t<SubmissionDate>".(string) DateFmt::dateTime(time(), $ctx->prefsStr('prefsTimeZone'), $ctx->prefsStr('prefsDateFormat'), $ctx->prefsStr('prefsTimeFormat'), 'long', $ctx->showTimezone())."</SubmissionDate>\n";
        $output .= '</OrgReport>';
